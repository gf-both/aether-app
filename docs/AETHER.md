# AETHER.md — Cosmic Agent Identity Standard
...
AETHER.md is a file placed in any AI agent's workspace that defines their complete cosmic identity.
Like SOUL.md defines personality and AGENTS.md defines behavior, **AETHER.md defines cosmic nature**.
...
Place `AETHER.md` in the agent's workspace root alongside `SOUL.md` and `AGENTS.md`:

```
workspace/
  AGENTS.md       ← behavior
  SOUL.md         ← personality
  AETHER.md       ← cosmic identity  ← you are here
```
...
|-------|------|-------------|
| `version` | string | AETHER spec version (currently `"1.0.0"`) |
...
These are derived from birth data by the AETHER engine — do not fill manually:
...
## How to Generate AETHER.md

### Option 1: Profile Designer UI
Open the AETHER tab in the Above + Inside app → Profile Designer → Generate.

### Option 2: API

```http
POST /api/aether/generate
...
```bash
openclaw aether generate --name "Agent Name" --role "Frontend Engineer" --relation complement
```
...
## AETHER + SOUL.md Integration

Your AETHER profile generates a `promptSnippet` — a compact identity block to inject into any prompt:
...
*AETHER is part of the Above + Inside ecosystem. The name stands for:*
