 ...
 * aetherApi.js
 *
 * High-level API for AETHER profile generation and agent assignment.
 * Uses aetherEngine locally — no external API calls needed.
 */

import { generateAetherProfile, generateAetherMd, CREATOR_BIRTH_DATA } from '../engines/aetherEngine.js'

// ─── Profile Generation ───────────────────────────────────────────────────────

/**
 * Generate a complete AETHER profile + markdown output.
 *
 * @param {object} params
...
export async function generateProfile(params) {
  ...
  const md = generateAetherMd(profile)
...
/**
 * Get Gaston's aether profile — used for mirror/complement visualization.
 */
export async function getCreatorProfile() {
  const { CREATOR_BIRTH_DATA: cbd } = await import('../engines/aetherEngine.js')
...
/**
 * Assign an AETHER profile to a Paperclip agent by injecting the promptSnippet
 * into the agent's heartbeat prompt.
 *
 * @param {string} agentId - Paperclip agent ID
 * @param {object} profile - AETHER profile object (from generateProfile)
...
// ─── List Agents with AETHER Profiles ────────────────────────────────────────

/**
 * Fetch all Paperclip agents and their AETHER profiles for the discussion board.
 *
 * @param {string} paperclipToken
 * @param {string} [paperclipBaseUrl]
 */
export async function listAgentsWithAether(paperclipToken, paperclipBaseUrl = 'https://api.paperclip.ai') {
  ...
  try {
    ...
    return (data.agents || []).filter(a => a.aether_profile)
  } catch {
    return []
  }
}
